...
class Settings:
    ...
    search_http_timeout: float = 30.0
    search_network_retries: int = 2
    search_ready_retries: int = 8
    search_ready_interval: float = 1.5

    def __post_init__(self) -> None:
        if not 1 <= self.worker_count <= 4:
            raise ConfigurationError("worker_count 必须在 1 到 4 之间")
    ...
    def from_env(cls) -> "Settings":
        ...
        worker_count = _positive_int("WORKER_COUNT", 4)
        if worker_count > 4:
            raise ConfigurationError("WORKER_COUNT 不能超过 4")
        ...
        return cls(
            ...
            task_timeout_seconds=_positive_float("TASK_TIMEOUT_SECONDS", 120.0),
            ...
            search_http_timeout=_positive_float("SEARCH_HTTP_TIMEOUT", 30.0),
            search_network_retries=_positive_int("SEARCH_NETWORK_RETRIES", 2),
            search_ready_retries=_positive_int("SEARCH_READY_RETRIES", 8),
